...
export function EmailVerificationEmail({ recipient, payload }: Props) {
  const verifyUrl = payload.data?.verifyUrl as string | undefined
        ...
        <Container style={{ maxWidth: 480, margin: '40px auto', background: '#ffffff', borderRadius: 8, overflow: 'hidden' }}>
          ...
          <Section style={{ padding: '32px' }}>
            ...
            <Text style={{ color: '#444', lineHeight: 1.6 }}>
              Olá, <strong>{recipient.name.split(' ')[0]}</strong>! Bem-vindo à ilecoffees.
            </Text>
            <Text style={{ color: '#444', lineHeight: 1.6 }}>
              Para ativar sua conta e começar a explorar nossos cafés especiais, confirme seu endereço de e-mail clicando no botão abaixo.
            </Text>
            {verifyUrl && (
              <Button
                href={verifyUrl}
            ...
            <Text style={{ color: '#888', fontSize: 12, margin: 0 }}>
              Se você não criou uma conta na ilecoffees, pode ignorar este e-mail com segurança.
            </Text>
          </Section>
        </Container>
      </Body>
