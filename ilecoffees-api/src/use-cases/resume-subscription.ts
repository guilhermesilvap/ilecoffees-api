...
export class ResumeSubscriptionUseCase {
  ...
  async execute(orderId: string, userId: string): Promise<Order> {
    ...
    if (order.subscriptionStatus !== 'PAUSED') throw new AppError('Assinatura não está pausada', 400)

    return this.ordersRepository.updateSubscriptionStatus(orderId, 'ACTIVE', null)
  }
}
