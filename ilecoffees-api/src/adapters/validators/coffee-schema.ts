...
export const coffeeCreateSchema = z
  .object({
    ...
    altitude: z.coerce.number().int().min(1),
    farm: z.string().min(2),
    producer: z.string().min(2),
    score: z.coerce.number().int().min(1),
    sensory: z.string().min(2),
    roast: z.string().min(2),
    saleType: z.enum(['KG', 'PACKAGE', 'BOTH']),
    pricePerKg: z.coerce.number().positive().optional(),
    packagePrice: z.coerce.number().positive().optional(),
    packagePriceCoffeeshop: z.coerce.number().positive().optional(),
...
export const coffeeUpdateSchema = z
  .object({
    ...
    altitude: z.coerce.number().int().min(1).optional(),
    farm: z.string().min(2).optional(),
    producer: z.string().min(2).optional(),
    score: z.coerce.number().int().min(1).optional(),
    sensory: z.string().min(2).optional(),
    roast: z.string().min(2).optional(),
    saleType: z.enum(['KG', 'PACKAGE', 'BOTH']).optional(),
    pricePerKg: z.coerce.number().positive().optional(),
    packagePrice: z.coerce.number().positive().optional(),
    packagePriceCoffeeshop: z.coerce.number().positive().optional(),
    packageWeight: z.coerce.number().positive().optional(),
...
export const coffeeQuerySchema = z.object({
  ...
  supplierType: z.enum(['PRODUCER', 'ROASTER']).optional(),
})
